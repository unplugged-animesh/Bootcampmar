        ...
        if 'user_id' in session and session['user_id'] == curr_login_id:
            ...
            data = {'curr_login_id': curr_login_id,
                    'cart': {f'{category.name}': [((lambda x: 0 if x is None else x.quantity)(CartItem.query.filter_by(
                        cartitem_product_id=product.id).first()), product) for product in category.products] for category in categories}}
...
@app.route('/admin/<int:curr_login_id>/edit_cat/<int:category_id>',methods=["GET","POST"])
def edit_category(curr_login_id,category_id):
    ...
    if request.method=="POST":
        try:
            ...
            db.session.rollback()
            return redirect(url_for('edit_category',curr_login_id=curr_login_id))
